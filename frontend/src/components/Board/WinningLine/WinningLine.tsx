...
const findWinningLine = (pattern: WinningPattern) => {
  const { winningLine } =
    winningsLine.find(({ winningRow }) => {
      return JSON.stringify(winningRow) === JSON.stringify(pattern);
    }) ?? {};
  return winningLine;
};
...
function WinningLine({ winningPattern }: WinningLineSvgProps) {
  return (
    <svg
      className={styles.winningLine__container}
      viewBox="0 0 100 100"
    >
      <line
        className={styles.winningLine__inner}
        {...findWinningLine(winningPattern)}
      />
    </svg>
  );
}
