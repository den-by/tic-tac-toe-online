...
function OnlineGameProvider({ children }: OnlineGameProviderProps) {
  ...
  const [currentGameInfoState, setCurrentGameInfo] = useState<OnlineGameProp>(currentGameInitialValues);
  const [boardState, setBoardState] = useState<BoardValues[]>(boardInitialValues);
  const [gameOverState, setGameOverState] = useState<GameOver>(gameOverInitialValues);
  ...
  const currentGameInfo = useMemo(() => currentGameInfoState, [currentGameInfoState]);
  const gameConversation = useMemo(() => gameConversationState, [gameConversationState]);
  ...
  const setGameOver = useCallback(({ isOver, winningPattern, isTie }: SetGameOverProps) => {
  ...
  const setGameConversation = useCallback(({ message, playerId }: Conversation) => {
